...
def start_upload(upload_data):
    ...
    post = fb_page.put_photo(open(img_content_path, "rb"), message=upload_data["title"], published= False, unpublished_content_type= "SCHEDULED", scheduled_publish_time= int(time.mktime(datetime.fromisoformat(upload_data["upload_datetimeiso"]).timetuple())))
    try: 
